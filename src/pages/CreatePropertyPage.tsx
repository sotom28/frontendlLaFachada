...
export function CreatePropertyPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: ['precio', 'cantidadBaños', 'cantidadHabitaciones', 'metraje', 'idCiudad', 'idTipoPropiedad'].includes(name) 
        ? Number(value) 
        : value,
    }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:3000/api/v1/publicacion/crear-completa', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setMessage({ type: 'success', text: data.message || 'Propiedad y publicación creadas exitosamente' })
        ...
        setMessage({ type: 'error', text: data.message || 'Ocurrió un error al procesar la solicitud. Por favor, inténtelo de nuevo.' })
        setLoading(false)
      }
    } catch (error) {
      setMessage({ type: 'error', text: 'Ocurrió un error al procesar la solicitud. Por favor, inténtelo de nuevo.' })
      setLoading(false)
    }
  }
    ...
    return (
      <main className="container">
        <div className="message-banner error" style={{ marginTop: '2rem' }}>
          No tienes permisos para acceder a esta página. Solo administradores pueden crear propiedades.
        </div>
      </main>
    )
  }
    ...
    <main className="container" style={{ padding: '2rem 1rem', position: 'relative' }}>
      {loading && (
        <div style={{
          ...
          <p style={{ fontWeight: '700', color: '#1e293b', fontSize: '1.3rem' }}>
            {message.type === 'success' ? '¡Éxito! Redirigiendo...' : 'Procesando tu publicación...'}
          </p>
          <style>{`
            @keyframes spin {
      ...
      <section className="auth-card" style={{ maxWidth: '900px', width: '100%', margin: '0 auto', padding: '32px' }}>
        <h1 style={{ textAlign: 'center', marginBottom: '8px' }}>Crear Nueva Propiedad</h1>
        ...
        <form className="auth-form" onSubmit={handleSubmit} style={{ gap: '24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
            
            <div style={{ gridColumn: '1 / -1' }}>
              <h3 style={{ borderBottom: '2px solid #f1f5f9', paddingBottom: '12px', marginBottom: '8px', color: '#1e293b' }}>
            ...
            <label>
              Tipo de Operación
              <select name="tipoVentas" value={formData.tipoVentas} onChange={handleChange}>
            ...
            <label>
              Ciudad
              <select name="idCiudad" value={formData.idCiudad} onChange={handleChange}>
            ...
            <label>
              Tipo de Propiedad
              <select name="idTipoPropiedad" value={formData.idTipoPropiedad} onChange={handleChange}>
