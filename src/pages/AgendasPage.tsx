...
export function AgendasPage() {
  ...
  const handleUpdateStatus = async (agendaId: number, nuevoEstado: string) => {
    console.log(
      "DEBUG: Intentando actualizar ID:",
      agendaId,
      "Estado:",
      nuevoEstado,
    );
    ...
    if (!agendaId) {
      alert(
        "Error: No se pudo identificar el ID de la agenda. Revisa la consola.",
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/v0/agenda/${agendaId}/estado`,
        {
          method: "PUT",
          headers: {
      ...
      alert("Error de conexión al intentar actualizar " + err);
    } finally {
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ color: "#64748b" }}>
              {user?.idRol === 1
                ? "Gestiona las citas de tus propiedades"
                : "Consulta tus próximas visitas a propiedades"}
            </p>
          </div>
          <Link
            to="/propiedades"
        ...
        {loading ? (
          ...
          >
            ...
            >
              Parece que aún no tienes citas pendientes. ¡Explora nuestras
              propiedades y agenda una visita hoy mismo!
            </p>
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        backgroundColor:
                          item.estadoCita?.toLowerCase() === "pendiente"
                            ? "#fef3c7"
                            : item.estadoCita?.toLowerCase() === "aceptada"
                              ? "#dcfce7"
                              : "#fee2e2",
                        color:
                          item.estadoCita?.toLowerCase() === "pendiente"
                  ...
                  >
                    <div>
                      ...
                      >
                        📍 {item.publicacion.ubicacion},{" "}
                        {item.publicacion.ciudad}
                      </p>

                      <div
                        style={{
                          display: "grid",
                          gridTemplateColumns: item.vendedorMensaje
                            ? "1fr 1fr"
                            : "1fr",
                          gap: "15px",
                          marginBottom: "15px",
                        ...
                        {item.vendedorMensaje && (
                          <div
                            style={{
                              padding: "12px",
                              backgroundColor:
                                item.estadoCita?.toLowerCase() === "cancelada"
                                  ? "#fef2f2"
                                  : "#f0fdf4",
                              borderRadius: "8px",
                          ...
                          >
                            ...
                            <div style={{ display: "flex", gap: "10px" }}>
                              <button
                                ...
                                onClick={() =>
                                  handleUpdateStatus(actualId, "ACEPTADA")
                                }
                                disabled={submittingIds.includes(actualId)}
                              >
                                {submittingIds.includes(actualId)
                                  ? "Procesando..."
                                  : "✅ Aceptar"}
                              </button>
                              <button
                                ...
                                onClick={() =>
                                  handleUpdateStatus(actualId, "CANCELADA")
                                }
                                disabled={submittingIds.includes(actualId)}
                              >
                                ❌ Cancelar
                    ...
                    >
                      ...
                      >
                        ...
                        <span style={{ fontSize: "0.9rem", color: "#334155" }}>
                          <span style={{ color: "#64748b" }}>
                            {item.participante.rol === "vendedor"
                              ? "Vendedor: "
                              : "Cliente: "}
                          </span>
                      ...
                      >
                        ...
                        <Link
                          to={`/propiedades/${item.publicacion.id}`}
